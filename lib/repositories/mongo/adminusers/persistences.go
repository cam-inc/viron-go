...
func (a *adminUsersPersistence) CreateOne(ctx context.Context, entity repositories.Entity) (repositories.Entity, error) {
	...
	// response を適切に使用するコードを追加
	result := &repositories.AdminUserEntity{}

	if err := a.client.Collection(collectionName).FindOne(ctx, bson.D{primitive.E{Key: "_id", Value: response.InsertedID}}).Decode(result); err != nil {
		return nil, err
	}
...
func (a *adminUsersPersistence) RemoveByID(ctx context.Context, id string) error {
	...
	// 削除
	if _, err = a.client.Collection(collectionName).DeleteOne(ctx, bson.D{primitive.E{Key: "_id", Value: oid}}); err != nil {
		return err
	}
	return nil
}
